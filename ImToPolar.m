function imP = ImToPolar (imR, rMin, rMax, M, N, centroid_vector)

% This function is used for the project.
% Code can be found at : https://fr.mathworks.com/matlabcentral/fileexchange/17933-polar-to-from-rectangular-transform-of-images
...
Om = centroid_vector(2); % co-ordinates of the center of the image
On = centroid_vector(1);
sx = (Mr - centroid_vector(2))/2; % scale factors
sy = (Nr - centroid_vector(1))/2;
imP  = zeros(M,  N);
delR = (rMax - rMin)/(M-1);
delT = 2*pi/N;
% loop in radius and 
